...
/**
 * @file plugins/paymethod/paystack/classes/SecretCipher.php
 *
 ...
 * @class SecretCipher
 *
 * @brief Symmetric encryption helper for at-rest secrets (Paystack API keys)
 * stored in `plugin_settings`. Prefers libsodium (secretbox / XSalsa20-Poly1305),
 * falling back to OpenSSL AES-256-GCM when sodium isn't available. Both are
 * bundled with PHP by default, so no new Composer dependency is introduced.
 *
 * Ciphertext is tagged with a short prefix identifying the scheme used, so
 * old plaintext values already in the database (written before this class
 * existed) are still readable: decrypt() returns them unchanged and the next
...
class SecretCipher
{
    private const PREFIX_SODIUM = 'psxsec1:';
    private const PREFIX_OPENSSL = 'psxsec2:';
     ...
     * (backward compatibility for rows written before encryption existed).
